...
private:
    ...
    bool isOverlap(const cv::Mat &mask) {
        for (auto &obs : obstacles) {
            ...
            if (cv::countNonZero(overlap) > 0) return true;
        }
        return false;
    }
...
public:
                ...
                std::unordered_map<Location,Location>& came_from)
    {
        ...
        while (start!=goal)
        {
            ...
            //是否超出边界
            if(new_loc.x < 0 || new_loc.x >= map.cols || new_loc.y < 0 || new_loc.y >= map.rows) continue;
            cv::Vec3b color = map.at<cv::Vec3b>(new_loc.y, new_loc.x);
            // 判断是否为障碍物
            if ( !isLineObstacleFree(map,new_loc,frontier.top().location) ) continue;
